...
export class Modal {
  ...
  // Get Country List. Try to get the translated version first
  loadCountries() {
    const metaState = document.querySelector('meta[name="state"]');
    let countries = "";
    if (metaState) {
      let siteSettings = JSON.parse(
        decodeURIComponent(metaState.getAttribute("content"))
      );
      if (
        siteSettings &&
        siteSettings.hasOwnProperty("settings") &&
        siteSettings.settings.hasOwnProperty("data") &&
        siteSettings.settings.data.hasOwnProperty("countries") &&
        siteSettings.settings.data.countries.length
      ) {
        siteSettings.settings.data.countries.forEach((element) => {
          let selected = element.selected ? "selected" : "";
          countries +=
            '<option value="' +
            element.alpha2 +
            '" ' +
  ...
  // Get the current region from the website
  getLocale() {
    const metaState = document.querySelector('meta[name="state"]');
    if (metaState) {
      let siteSettings = JSON.parse(
      ...
      if (
        siteSettings &&
        siteSettings.hasOwnProperty("i18n") &&
        siteSettings.i18n.hasOwnProperty("paramsLocaleCode")
      ) {
        return siteSettings.i18n.paramsLocaleCode;
      }
    }
    return "us";
  }
